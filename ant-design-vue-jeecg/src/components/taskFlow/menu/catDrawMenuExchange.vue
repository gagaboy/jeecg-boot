<template>
	<div>
		...
		<div class="draw-side-menu">
			<menuCommonTop @catBackButton="catBackButton" :showBackButton="showBackButton" topTitle="任务切换动作"></menuCommonTop>
            ...
            <span class="txt-limit" >{{nowNode.nodeName && nowNode.nodeName.length || 0}} / 25</span>
				...
				<div class="draw-side-menu-trigger-title">执行要执行的任务</div>
				<a-select :default-value="nowNode.exchangeId ? { key: nowNode.exchangeId } : undefined" placeholder="请选择" style="width:352px" @change="onChange" label-in-value :getPopupContainer="triggerNode => { return triggerNode.parentNode }">
					<a-select-option v-for="item in workFlows" :value="item.value" :key="item.value">{{ item.label }}</a-select-option>
				</a-select>
...
<script>
	...
	export default {
		...
		methods: {
			onChange(obj) {
				this.nowNode.exchangeId = obj.key
				this.nowNode.exchangeName = obj.label //设置value的值
			},
			clickAll(e) {
			...
			decide() {
				...
				if(!this.nowNode.exchangeId) {
					this.$message.warning('任务切换不能为空')
					return false
				}

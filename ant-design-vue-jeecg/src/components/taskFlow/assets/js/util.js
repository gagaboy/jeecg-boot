...
var getUUID = () => {
	...
	var str = Math.random().toString(36).substr(3, 10)  //document.querySelector不能获取以数字开头的,所以用这个的方法
	return 'a' + str
}
...
function showEllipsis(str, maxLen = 32) {
				var that = this
				var result = str
				if(getStrRealLength(str) > maxLen) {
					...
					var needLen = maxLen - getStrRealLength(lastStr1) - 3  //3个省略号
					var firstStr = cutStrReal(str, needLen)
					result = firstStr + '...' + lastStr1
				}
				return result
		}
...
var auditStatusList = {
	'1': {
		title: '审核中...',
		style: {
			background: '#3C64B9',  //catChoose组件中用到
		},
		rowStyle: {},
	},
	'2': {
		title: '审核不通过',
		style: {
			background: '#D35656', //catChoose组件中用到
		},
		rowStyle: {
			color: '#F5222D',  //catMaterialList > catMaterialListRow组件有用到
		},
	},
	'3': {
		title: '审核通过',
		style: {
			background: '#52BB90', //catChoose组件中用到
		},
		rowStyle: {
			color: '#52C41A',  //catMaterialList > catMaterialListRow组件有用到
		},
	},
	'4': {
		title: '待审核',
		style: {
			background: '#E6A23C', //catChoose组件中用到
		},
		rowStyle: {},
	},
}
...
//0728新增判断2个日期的大小
function calendarDate(date1, date2) {
	var oDate1 = new Date(date1);
	var oDate2 = new Date(date2);
	return oDate1.getTime() > oDate2.getTime() //转换成毫秒进行比较
}

//0817新增查找元素
var querySelector = function (sel) {
	return document.querySelector(sel)
}

//0817新增判断是空对象
var judgeEmptyObj = (data) => {
	if (!data) {
		return false // 如果为空,返回false
	}
	if (JSON.stringify(data) === '{}') {
		return false // 如果为空对象,返回false
	}
	return true // 如果不为空，则会执行到这一步，返回true
}

//0820新增判断是否是string,否则就转成json.兼容之前的数据
var convertToJson = function (data) {
	if (typeof data == 'string') {
		return JSON.parse(data) //转成json
	} else {
		return data
	}
}

//0619新增uuid方法--https://www.jianshu.com/p/b6c32ad2ab01
function getUUIDnew() {
    var s = [];
...
export {
	...
	calendarDate, //0728新增判断2个日期的大小

	querySelector,  //0817新增查找元素
	judgeEmptyObj,  //0817新增判断是空对象
	convertToJson,  //0820新增判断是否是string,否则就转成json.兼容之前的数据

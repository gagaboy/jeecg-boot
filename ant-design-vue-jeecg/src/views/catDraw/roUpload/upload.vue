<template>
    <div :class="[prefixCls]">
        <div
            ...
            <!--0628新增上传封面图-->
            <div v-if="showValidateMsg" class="flex-center" style="width: 278px; height: 166px; flex-direction: column; background: #FAFAFA;">
								...
								<!--上传前-->
								<template v-else>
									<!-- <i class="el-icon-circle-plus-outline" style="color: #260000;font-size:52px;"></i> -->
                                    <a-icon type="plus" style="color: #260000;font-size:52px;"/>
									<p class="upload-title">请上传封面图</p>
								</template>
							</div>
        ...
        <upload-list
            ...
            <!-- <el-button v-if="!autoUpload  && fileList && fileList.length" :disabled="isdisabled"  size="mini" @click.native="startUpload">一键上传按钮</el-button> -->
            <a-button v-if="!autoUpload  && fileList && fileList.length" :disabled="isdisabled" size="small" @click.native="startUpload"> 一键上传按钮</a-button>
            <!--<input :form="form" :name="sendKey" type="hidden" v-model="uploadSuccessFile"/>-->
            <!--0623新增绑定公共组件里的提交-->
            <template v-if="showValidateMsg">
	            <input :form="form" type="hidden" :name="sendKey" size="100" v-model="uploadSuccessFile[0]" v-validate="validate" :data-vv-as="fieldTitle" :data-vv-name="sendKey" />
	            <span v-if="errors.has(sendKey)" class="m-form-error-tip">
				        {{ validateMsg?validateMsg:errors.first(sendKey) }}
				    	</span>
			    	</template>
			    	<input v-else :form="form" :name="sendKey" type="hidden" v-model="uploadSuccessFile"/>
            ...
            fileList: {
                ...
                handler(fileList) {
                	...
                	fileList.forEach((file) => {
                		if(file.status === 'finished' && file.response && file.response.result) {

	...
	<ul :class="[prefixCls + '-list']" v-bind:style="{'max-width': maxWidth || 'auto'}">
		<li v-for="(file, index) in files" :key="index" :class="fileCls(file)" @click="handleClick(file)">
			...
			<div class="el-upload-button">
				<!--重新上传,失败时显示按钮-->
				<Icon v-if="file.status === 'fail'" class="icon" type="ios-refresh-empty" size="16" @click.native="handleRefresh(file)"></Icon>
				<!--上传按钮，首次添加的时候显示按钮-->
				<Icon v-if="file.status === 'noupload'" class="icon" type="ios-cloud-upload-outline" size="16" @click.native="handleUpload(file)"></Icon>
				<Icon type="ios-close" class="icon" size="16" @click.native="handleRemove(file)"></Icon>
			</div>
			<!--进度条-->
			<transition name="fade">
				<i-progress v-if="file.showProgress && file.percentage" :stroke-width="2" :percent="parsePercentage(file.percentage)" :status="progressStatus(file)"></i-progress>
			</transition>
		</li>
	</ul>
</template>

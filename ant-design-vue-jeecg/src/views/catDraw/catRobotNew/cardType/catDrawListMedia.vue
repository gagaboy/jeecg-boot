<template>
	<div :style="listStyle">
		<div class="flex-center instance-top-content">
				...
				<ro-upload :disabled="!pageParameters.needUpload" type="drag" field="mediaUrl" ref="upload" :show-upload-list="false" :on-success="handleSuccess" :data="{'mediaType': uploadType.mediaType}" :format="uploadType.format" :accept="uploadType.accept" :max-size="uploadType.maxSize" sendKey="ajaxUpload" :before-upload="handleUpload" id="ImportTargetForm_fileUrl" fieldTitle="上传文件" :validate="'required'" :deleteFailFile="true" :autoUpload="true" :action="acceptUrl.uploadFiles">
					<div @click="openMaterialModal" class="flex-center" style="width: 278px; height: 166px; flex-direction: column; background: #FAFAFA;">
						<template v-if='item.reply && item.reply.fileName'>
							<div class="upload-img" :style="item.replyType === 'file'?{width: '60px',height: '60px'}:{}">
								<img v-if="item.replyType == 'image'" class="upload-img" :src="item.reply.cspMediaUrl || item.reply.imosMediaUrl || item.reply.mediaUrl" />
								<img v-else-if="item.replyType == 'audio'" class="upload-img" src="~@/assets/img/audioIcon.png" />
								<img v-else-if="item.replyType == 'file'" class="upload-img" style="width: 60px; height: 60px;" src="~@/assets/img/fileIcon.jpg" />
								<!--如果是视频，需显示封面图-->
								<img v-else-if="item.replyType == 'video' && item.reply.thumbnailMediaUrl" class="upload-img" :src="item.reply.cspThumbnailMediaUrl || item.reply.thumbnailMediaUrl"></img>
								<!-- 视频蒙层 -->
			          <div v-if="item.replyType == 'video'" class="play-icon">
						...
						<template v-else >
							<!--<Icon type="ios-plus-empty" size="52" style="color: #260000"></Icon>
								<p class="upload-title">{{uploadType.tishiTitle}}</p>
								<p class="upload-title" style="line-height:22px; width: 221px;">{{uploadType.tishi}}</p>-->
							<!-- <Icon type="ios-plus-empty" size="52" style="color: #260000"></Icon> -->
							<a-icon type="plus" style="margin-right: 6px;font-size:30px;"/>
								...
								<p class="upload-title" style="line-height:22px; width: 221px;">{{pageParameters.needUpload ? uploadType.tishi : ''}}</p>
						</template>
					</div>
				</ro-upload>
			</div>
